...
| Slate scale (dark:)      | dark:bg-slate-800, dark:text-slate-200                  | same scale, `dark:` variant — gated on the viewer's OS colour-scheme setting, not this stylesheet's `data-theme` toggle; see note below                                                  |
...
## `dark:` utilities track the OS, not this page's theme

No `@custom-variant dark` is declared anywhere in this stylesheet (nor in the
live site's `global.css` it is copied from), so Tailwind v4's default `dark:`
variant applies: every `.dark\:*` utility compiles inside
`@media (prefers-color-scheme: dark)`. That means `dark:` utilities are gated
on the **viewer's OS colour-scheme setting**, not on the page's own theme.

This stylesheet's own theming is a separate mechanism: dark is the default in
`:root`, with an explicit `[data-theme='light']` override applied when the
page switches to light mode. A viewer on a light OS who has not toggled the
page to light mode gets the page's dark `:root` values but _skips_ every
`dark:` utility, and vice versa — the two switches are independent and can
disagree.

To style against the page's own theme (the one the toggle actually changes),
use the CSS custom properties directly — `var(--bg-surface)`,
`var(--text-primary)`, `var(--border-color)`, etc. — rather than `dark:`
utilities. Reach for `dark:` only when you deliberately want to follow the
OS setting instead of the page toggle.
