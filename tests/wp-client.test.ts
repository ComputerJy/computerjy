...
describe('fetchAllPaginated', () => {
  it('concatenates every page', async () => {
    ...
    const out = await fetchAllPaginated<{ id: number }>(
      'posts',
      'id',
      f as typeof fetch
    );
  ...
  it('throws when the assembled count does not match X-WP-Total', async () => {
    ...
    ).rejects.toThrow(WpApiError);
    await expect(
      fetchAllPaginated('posts', 'id', f as typeof fetch)
    ).rejects.toThrow(/returned 3 items but X-WP-Total reported 99/);
  });
  ...
  it('throws on a network error rather than returning empty', async () => {
    ...
    await expect(
      fetchAllPaginated('posts', 'id', f as typeof fetch)
    ).rejects.toThrow(/Network failure fetching .*ECONNREFUSED/);
  });
...
describe('fetchByIds', () => {
  it('returns all requested items when every id resolves', async () => {
    ...
    const f = async (url: string | URL): Promise<Response> => {
      ...
      return new Response(JSON.stringify([{ id: 3243 }, { id: 3234 }]), {
        status: 200,
      });
    };
    const out = await fetchByIds<{ id: number }>(
      'media',
      'id',
      [3243, 3234],
      f as typeof fetch
...
describe('WP_BUILD_TOKEN header', () => {
  ...
  it('sends X-Build-Auth when WP_BUILD_TOKEN is set', async () => {
    ...
    await fetchAllPaginated(
      'posts',
      'id',
      capturingFetch(seen) as typeof fetch
    );
    expect((seen[0]?.headers as Record<string, string>)?.['X-Build-Auth']).toBe(
      'sekrit'
    );
  });
