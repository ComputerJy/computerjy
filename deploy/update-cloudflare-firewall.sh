...
echo ""
echo "That file also feeds Apache's mod_remoteip trusted-proxy list. Regenerate and"
echo "re-apply it, or Apache stops trusting CF-Connecting-IP from any new range and"
echo "those requests quietly go back to logging the Cloudflare edge IP:"
echo "  bash deploy/generate-remoteip-conf.sh > deploy/lightsail-remoteip.conf"
echo "  sudo install -m 644 deploy/lightsail-remoteip.conf /etc/apache2/conf-available/remoteip.conf"
