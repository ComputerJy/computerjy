...
Let's Encrypt reached the origin over port 80 _through_ Cloudflare, so renewal only
...
Under the old `authenticator = apache`, certbot's Apache _installer_ reloaded the
...
The monitor also treats _its own_ failure to read the certificate as an alert. A
